...
def process_frame(frame, model, iou_thresh, draw_bounding_box):
    ...
    roi_corners_right = np.array([vertices_right], dtype=np.int32)
    cv2.fillPoly(mask_right, roi_corners_right, (255, 255, 255))
    cropped_frame_left = cv2.bitwise_and(frame, mask_left)
    cropped_frame_front = cv2.bitwise_and(frame, mask_front)
    cropped_frame_right = cv2.bitwise_and(frame, mask_right)
    ...
    result_right = model(cropped_frame_right, mode='predict', classes=[0], iou=iou_thresh, hide_conf=True, save=True, conf = 0.1 )

    # Count number of people detected
    num_people_left = len(result_left[0].boxes)
    num_people_front = len(result_front[0].boxes)
    num_people_right = len(result_right[0].boxes)
    ...
    if draw_bounding_box:
        ...
        for i, det in enumerate(result_front[0].boxes):
            ...
            cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)
            cv2.putText(frame, label, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        for i, det in enumerate(result_right[0].boxes):
            x1, y1, x2, y2 = det.xyxy[0].tolist()
            label = f"Person {i + 1}"
            cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)
            cv2.putText(frame, label, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    cv2.polylines(frame, [roi_corners_right], True, (0, 255, 0), thickness=2)
    ...
    cv2.putText(frame, f"Right side: {num_people_front}", (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 4)
